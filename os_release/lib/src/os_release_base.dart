...
final class OsReleaseInfo{
  ...
  OsReleaseInfo({required this.name, this.version, this.versionId, this.cpeName, this.variant, this.variantId, this.buildId,
  ...
  static OsReleaseInfo detect(){
    if(Platform.isLinux){
      ...
      for(int index = 0; index < lines.length; index++){
        ...
        if(currentLine.toUpperCase().contains("ID")){
          ...
          if(currentLine.toUpperCase().contains("LIKE")){
            identifierLike = currentLine.replaceFirst("ID_LIKE=", "").split(" ");
          }
        ...
        else if(currentLine.toUpperCase().contains("VARIANT") && !currentLine.toUpperCase().contains("ID")){
          variant = currentLine.replaceFirst("VARIANT=", "");
        }

        if(currentLine.toUpperCase().contains("IMAGE") && !currentLine.toUpperCase().contains("VERSION")){
        ...
        if(currentLine.toUpperCase().contains("SUPPORT_END")){
          List<String> dateSplit = currentLine.replaceFirst("SUPPORT_END=", "").split("-");
          
          supportEnd = DateTime(int.parse(dateSplit[0]), int.parse(dateSplit[1]), int.parse(dateSplit[2]));
        }
        
        if(currentLine.toUpperCase().contains("VERSION") && !currentLine.toUpperCase().contains("ID=")){
        ...
        if(currentLine.toUpperCase().contains("URL")){
          if(currentLine.toUpperCase().contains("HOME_")){
            ...
            privacyPolicyUrl = currentLine.replaceFirst("PRIVACY_POLICY_URL=", "");
          }
          else if(currentLine.toUpperCase().contains("DOCUMENTATION")){
            documentationUrl = currentLine.replaceFirst("DOCUMENTATION_URL=", "");
          }
      ...
      return OsReleaseInfo(name: name, version: version, identifier: identifier, identifierLike: identifierLike,
          prettyName: prettyName, versionCodename: versionCodename,  versionId: versionId, homeUrl: homeUrl,
          supportUrl: supportUrl, bugReportUrl: bugReportUrl, privacyPolicyUrl: privacyPolicyUrl,
          isLongTermSupportRelease: isLongTermSupportRelease, documentationUrl: documentationUrl, logo: logo,
        ansiColor: ansiColor, vendorUrl: vendorUrl, vendorName: vendorName, supportEnd: supportEnd, cpeName: cpeName,
          variant: variant, variantId: variantId, buildId: buildId, imageId: imageId, imageVersion: imageVersion,
      defaultHostname: defaultHostname, portablePrefixes: portablePrefixes, confextLevel: confextLevel,
        confextScope: confextScope, architecture: architecture, sysExtLevel: sysExtLevel, sysExtScope: sysExtScope);
